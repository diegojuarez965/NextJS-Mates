...
import { updateMate, MateState } from '@/app/lib/actions';
import { useEffect, useState, useActionState } from 'react';
import { cloudinaryWidgetConfig } from '@/app/lib/cloudinary-config';
import { Listbox } from '@headlessui/react';
import Image from 'next/image';

export default function EditMateForm(
  mate: Mate,
) {
  const initialState: MateState = { message: null, errors: {} };
  const updateMateWithId = updateMate.bind(null, mate.id);
  // Manejo de estado del formulario mediante acción del servidor
  const [state, formAction] = useActionState(updateMateWithId, initialState);
  const [urlImage, setUrlImage] = useState(mate.imagen);
  const [selected, setSelected] = useState(mate.estado);
